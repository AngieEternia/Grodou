...
module.exports = {
    ...
    async runInteraction(client, interaction) {
        ...
        if (!cmdName) {
            ...
            let row = new MessageActionRow(), options = categories.map(v => {
                return {
                    label: (v.name).toUpperCase(),
                    description: v.description,
                    emoji: v.emoji,
                    value: (v.position - 1).toString(),
                }
            })
            ...
            options.push(options2) // On push cette option de fin à la fin de l'array option
            ...
            options.splice(0, 0, options3); // on intercale cette option au tout début du array option

            row.addComponents({
                type: "SELECT_MENU",
                customId: "select",
                placeholder: "Choisissez la catégorie ci-dessous...",
                options
            });

            let msg = await interaction.reply({ embeds: [menuEmbed], components: [row], fetchReply: true })
            ...
            for (let i = 0; i < categories.length; i++) {
                ...
                const thumb = new MessageAttachment(`./Img/help/g_${catLink.trim()}.png`, `miniature.png`);
        ...
        // Si l'utilisateur a indiqué une commande en argument
            ...
            const thumbnail = new MessageAttachment(`./Img/help/g_${catLinkMinia}.png`, `miniature.png`);
